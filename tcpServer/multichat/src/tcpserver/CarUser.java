...
public class CarUser extends Thread{
	Socket client;
	...
	public CarUser(Socket client,Vector<User> userlist,Vector<CarUser> carlist,ServerView serverView,String info,String ip) {
		this.client = client;
	...
	public void ioWork() { //처음 접속했을 때 한 번 실행되는 메소드
		try {
			...
			info = br.readLine();
			if(info.equals("car")) {
				serverView.taclientlist.append("========차유저  발동!!!\n");
				carlist.add(this);
			}
