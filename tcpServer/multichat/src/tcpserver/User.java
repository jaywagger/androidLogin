...
public class User extends Thread{
	...
	public User(Socket client,Vector<User> userlist,Vector<User> carlist,ServerView serverView,String info,String ip) {
		this.client = client;
	...
	public void ioWork() { //처음 접속했을 때 한 번 실행되는 메소드
		...
		//	System.out.println("폰 :"+userlist.size()+userid);
		//	System.out.println("카 :"+carlist.size()+userid);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void run() {
		...
		try {
			info = br.readLine();
			serverView.taclientlist.append(ip+" "+info+": 사용자========입장  !!!\n");
